...
export const LeagueTable: React.FC<LeagueTableProps> = ({ leagueName, teams, headerColor = 'text-slate-200' }) => {
  ...
  return (
    <div className="bg-[#323232] rounded-lg border border-white/10 overflow-hidden shadow-lg">
      <div className="bg-black/20 px-4 py-3 border-b border-white/10 flex justify-between items-center">
          ...
          {sortedTeams.map((team, index) => {
            ...
            return (
              <tr key={team.id} className="hover:bg-white/5 transition-colors group">
                ...
                <td className="px-3 py-2 text-center font-mono text-slate-400 text-xs">{team.division.substring(0, 1)}</td>
                <td className="px-3 py-2 text-center font-mono text-slate-200">{team.wins}</td>
                <td className="px-3 py-2 text-center font-mono text-slate-400">{team.losses}</td>
                <td className="px-3 py-2 text-center font-mono text-slate-300">{pct}</td>
                <td className="px-3 py-2 text-center font-mono text-slate-500 text-xs">{gb}</td>
                <td className={`px-3 py-2 text-center font-mono text-xs font-bold ${diff > 0 ? 'text-emerald-400' : diff < 0 ? 'text-rose-400' : 'text-slate-500'}`}>
                  {diff > 0 ? '+' : ''}{diff}
                </td>
                <td className="px-3 py-2 text-center font-mono text-slate-500 text-xs">-</td>
                <td className="px-3 py-2 text-center font-mono text-slate-500 text-xs">-</td>
              </tr>
            );
          })}
